...
async function fetchDemoFile(key) {
  ...
  try {
    ...
    if (response.status != 200) throw new Error(req.status + " Unable to load " + req.url);
...
async function parseReaderToJson(reader) {
...
const asset = getQueryParam('asset');
...
async function main() {
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    pausePlayButton.addEventListener('click', () => {
      isPaused = !isPaused;
      console.log("Pause state changed. isPaused:", isPaused);
      console.log('viewMatrix', viewMatrix);
      if (isPaused) {
        pauseTime = performance.now();
        playPauseIcon.setAttribute('d', 'M8,5.14V19.14L19,12.14L8,5.14Z');
      } else {
        const timeDifference = performance.now() - pauseTime;
        start += timeDifference;
        lastFrameTime = performance.now();
        playPauseIcon.setAttribute('d', 'M5,5.14V19.14H9V5.14H5M15,5.14V19.14H19V5.14H15Z');
      }
  });
  ...
  const frame = (now) => {
    ...
    if (!isPaused) {
      if (lastFrameTime !== 0) {
        const deltaTime = (rightNow - lastFrameTime) / 1000;
    ...
    // When paused, only update camera position
    viewMatrix = invert4(inv);
  }
    ...
    if (vertexCount > 0) {
      ...
      gl.uniform1f(u_time, (animationTime / 2) % 1);
...
main().catch((err) => {
  console.log("error hybrid.js!", err)
